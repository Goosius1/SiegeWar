...
public class ResidentMetaDataController {
	...
	private static IntegerDataField legacyFieldPlunder = new IntegerDataField("siegewar_plunder", 0); //Field no longer in use
	private static StringDataField legacyFieldRecentBattleSessions = new StringDataField("siegewar_recentbattlesessions", "");
	...
	/**
	 * This method is used to remove legacy metadata fields
	 * @param resident a resident
	 */
	public static void removeLegacyFields(Resident resident) {
		IntegerDataField idf = (IntegerDataField) legacyFieldPlunder.clone();
		if (resident.hasMeta(idf.getKey())) {
			resident.removeMetaData(idf);
		}
		StringDataField sdf = (StringDataField) legacyFieldRecentBattleSessions.clone();
